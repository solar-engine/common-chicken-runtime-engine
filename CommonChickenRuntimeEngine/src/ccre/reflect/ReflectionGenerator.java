...
public class ReflectionGenerator {
    ...
    private void recurseFinding(File f, String base) throws ClassNotFoundException {
        if (f.isDirectory()) {
            File[] fls = f.listFiles();
            Arrays.sort(fls);
            for (File i : fls) {
                recurseFinding(i, base);
            }
            ...
            this.toProcess.add(Class.forName(cname));
        }
    }
    ...
    public static void main(String[] args) throws ClassNotFoundException, IOException {
        ...
        test.recurseFinding(f, f.toString());
        test.runAll();
        test.end();
        if (args.length > 0) {
            test.printAll(new FileOutputStream(args[0]));
        } else {
            test.printAll(System.out);
        }
    }
