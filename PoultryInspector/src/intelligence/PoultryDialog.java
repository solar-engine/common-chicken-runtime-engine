...
public final class PoultryDialog {
    ...
    public static final int DEFAULT_WIDTH = 400, DEFAULT_HEIGHT = 300,
            BUTTON_HEIGHT = 50, BUTTON_PADDING = 25, BUTTONS_PER_LINE = 3;
    ...
    public boolean isOver(int basex, int swidth, int sheight, int tx, int ty) {
        int centerX = (basex + swidth) / 2, centerY = sheight / 2;
        return centerX - DEFAULT_WIDTH / 2 <= tx && tx <= centerX + DEFAULT_WIDTH / 2 && centerY - DEFAULT_HEIGHT / 2 <= ty && ty <= centerY + DEFAULT_HEIGHT / 2;
    }
