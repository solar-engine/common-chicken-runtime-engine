...
public class CANTalon {
     ...
     * by Talon's 100ms timebase to rotations per minute.
     */
    ...
    /**
     * Enumerated types for Motion Control Set Values. When in Motion Profile
     * control mode, these constants are paseed into set() to manipulate the
     * motion profile executer. When changing modes, be sure to read the value
     * back using getMotionProfileStatus() to ensure changes in output take
     * effect before performing buffering actions. Disable will signal Talon to
     * put motor output into neutral drive. Talon will stop processing motion
     * profile points. This means the buffer is effectively disconnected from
     * the executer, allowing the robot to gracefully clear and push new traj
     * points. isUnderrun will get cleared. The active trajectory is also
     * cleared. Enable will signal Talon to pop a trajectory point from it's
     * buffer and process it. If the active trajectory is empty, Talon will
     * shift in the next point. If the active traj is empty, and so is the
     * buffer, the motor drive is neutral and isUnderrun is set. When active
     * traj times out, and buffer has at least one point, Talon shifts in next
     * one, and isUnderrun is cleared. When active traj times out, and buffer is
     * empty, Talon keeps processing active traj and sets IsUnderrun. Hold will
     * signal Talon keep processing the active trajectory indefinitely. If the
     * active traj is cleared, Talon will neutral motor drive. Otherwise Talon
     * will keep processing the active traj but it will not shift in points from
     * the buffer. This means the buffer is effectively disconnected from the
     * executer, allowing the robot to gracefully clear and push new traj
     * points. isUnderrun is set if active traj is empty, otherwise it is
     * cleared. isLast signal is also cleared.
     *
     * Typical workflow: set(Disable), Confirm Disable takes effect, clear
     * buffer and push buffer points, set(Enable) when enough points have been
     * pushed to ensure no underruns, wait for MP to finish or decide abort, If
     * MP finished gracefully set(Hold) to hold position servo and disconnect
     * buffer, If MP is being aborted set(Disable) to neutral the motor and
     * disconnect buffer, Confirm mode takes effect, clear buffer and push
     * buffer points, and rinse-repeat.
     */
    ...
    private static long millisDelayForSolicitedSignals = 4;
    int m_deviceNumber;
     ...
     * Period is bounded to [1ms,95ms]. This typically is not required to
     * specify, however this could be used to minimize the time between
     * robot-enable and talon-motor-drive.
     */
    ...
    public void set(double outputValue) {
        if (m_controlEnabled) {
            ...
            case PercentVbus:
                CanTalonJNI.Set(m_handle, outputValue);
                break;
            case Follower:
            ...
            case Voltage:
                ...
                break;
            case Speed:
                CanTalonJNI.SetDemand(m_handle, ScaleVelocityToNativeUnits(m_feedbackDevice, (outputValue)));
                break;
            case Position:
                CanTalonJNI.SetDemand(m_handle, ScaleRotationsToNativeUnits(m_feedbackDevice, outputValue));
                break;
            case Current:
                double milliamperes = (outputValue) * 1000.0; /* mA */
     ...
     * profile. mA for Curent closed loop. Talon Native Units for position and
     * velocity.
     */
    ...
    public void configEncoderCodesPerRev(int codesPerRev) {
         ...
         * can report it to whoever needs it, like the webdash. Don't bother
         * checking the return, this is only for instrumentation and is not
         * necessary for Talon functionality.
         */
    ...
    public void setFeedbackDevice(FeedbackDevice device) {
        /*
         * save the selection so that future setters/getters know which scalars
         * to apply
         */
    ...
    public double getP() {
        ...
        try {
            Thread.sleep(millisDelayForSolicitedSignals);
    ...
    public void ConfigFwdLimitSwitchNormallyOpen(boolean normallyOpen) {
        CanTalonJNI.SetParam(m_handle, CanTalonJNI.param_t.eOnBoot_LimitSwitch_Forward_NormallyClosed.value, normallyOpen ? 0 : 1);
    }
    ...
    public void ConfigRevLimitSwitchNormallyOpen(boolean normallyOpen) {
        CanTalonJNI.SetParam(m_handle, CanTalonJNI.param_t.eOnBoot_LimitSwitch_Reverse_NormallyClosed.value, normallyOpen ? 0 : 1);
    }
    ...
    double GetNativeUnitsPerRotationScalar(FeedbackDevice devToLookup) {
        ...
        switch (devToLookup) {
        case QuadEncoder: { /*
                             * When caller wants to lookup Quadrature, the QEI
                             * may be in 1x if the selected feedback is edge
                             * counter. Additionally if the quadrature source is
                             * the CTRE Mag encoder, then the CPR is known. This
                             * is nice in that the calling app does not require
                             * knowing the CPR at all. So do both checks here.
                             */
            ...
            default: /*
                      * pulse width and everything else, assume its regular quad
                      * use.
                      */
                break;
            }
     ...
     * rotations of selected feedback sensor. If user has never called the
     * config routine for the selected sensor, then the caller is likely passing
     * rotations in engineering units already, in which case it is returned as
     * is.
     ...
     * minute of selected feedback sensor. If user has never called the config
     * routine for the selected sensor, then the caller is likely passing
     * rotations in engineering units already, in which case it is returned as
     * is.
     * @see configPotentiometerTurns
     * @see configEncoderCodesPerRev
     ...
     * SRX units. If user has never called the config routine for the selected
     * sensor, then the return will be in TALON SRX units as well to match the
     ...
     * rising edge, pass false to clear the position on falling edge.
     */
     ...
     * velocity should be used, i.e. do not perform PID on position. This is
     * equivalent to setting PID gains to zero, but much more efficient and
     * synchronized to MP.
     * @param isLastPoint set to nonzero to signal Talon to keep processing this
     * trajectory point, instead of jumping to the next one when timeDurMs
     * expires. Otherwise MP executer will eventuall see an empty buffer after
     * the last point expires, causing it to assert the IsUnderRun flag. However
     * this may be desired if calling application nevers wants to terminate the
     * MP.
     * @param zeroPos set to nonzero to signal Talon to "zero" the selected
     * position sensor before executing this trajectory point. Typically the
     * first point should have this set only thus allowing the remainder of the
     * MP positions to be relative to zero.
     ...
     * currently running MP. Caller should must instantiate the
     * motionProfileStatus object first then pass into this routine to be
     * filled.
     */
    ...
    protected void setMotionProfileStatusFromJNI(MotionProfileStatus motionProfileStatus, int flags, int profileSlotSelect, int targPos, int targVel, int topBufferRem, int topBufferCnt, int btmBufferCnt, int outputEnable) {
        ...
        motionProfileStatus.activePoint.zeroPos = false; // this signal is only
                                                         // used sending pts to
                                                         // Talon
